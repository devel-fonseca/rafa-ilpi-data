...
/**
 * Sugere código alternativo se houver conflito
 */
export function suggestAlternativeCode(
  baseCode: string,
  existingCodes: string[],
  maxLength: number = 2
): string {
  // Se o código é único, retorna ele mesmo
  if (!existingCodes.includes(baseCode)) {
    return baseCode
  }

  // Tenta adicionar números
  for (let i = 1; i <= 9; i++) {
    const alt = baseCode.substring(0, maxLength - 1) + i
    if (!existingCodes.includes(alt)) {
      return alt
    }
  }

  // Tenta trocar segunda letra
  const first = baseCode[0]
  for (let i = 65; i <= 90; i++) {
    const letter = String.fromCharCode(i)
    const alt = first + letter
    if (alt !== baseCode && !existingCodes.includes(alt)) {
      return alt
    }
  }

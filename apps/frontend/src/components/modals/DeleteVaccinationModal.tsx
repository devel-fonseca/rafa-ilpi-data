...
// ──────────────────────────────────────────────────────────────────────────────
//  MODAL - DeleteVaccinationModal (Exclusão de Vacinação com Reautenticação)
// ──────────────────────────────────────────────────────────────────────────────
...
import type { Vaccination } from '@/api/vaccinations.api'
import { useQueryClient } from '@tanstack/react-query'
import { useToast } from '@/components/ui/use-toast'

// ========== VALIDATION SCHEMA ==========

const deleteVaccinationSchema = z.object({
  deleteReason: z
    .string()
...
type DeleteVaccinationFormData = z.infer<typeof deleteVaccinationSchema>
...
export function DeleteVaccinationModal({ vaccination, open, onOpenChange, onSuccess }: DeleteVaccinationModalProps) {
  // Hooks
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<DeleteVaccinationFormData>({
    resolver: zodResolver(deleteVaccinationSchema),
  })

  // ========== HANDLERS ==========

  const onSubmit = async (data: DeleteVaccinationFormData) => {
    if (!vaccination) return

    setIsDeleting(true)
    try {
      await vaccinationsAPI.remove(vaccination.id, data.deleteReason.trim())

      // Sucesso
      toast({
        title: 'Vacinação excluída',
        description: 'O registro de vacinação foi excluído com sucesso.',
        variant: 'default',
      })

      // Invalidar queries
      queryClient.invalidateQueries({ queryKey: ['vaccinations'] })
      queryClient.invalidateQueries({ queryKey: ['vaccination-history'] })
      ...
      toast({
        ...
        description: error.response?.data?.message || 'Ocorreu um erro ao excluir o registro de vacinação.',
        variant: 'destructive',
      })
    } finally {
      setIsDeleting(false)
    ...
    <>
      <AlertDialog open={open} onOpenChange={onOpenChange}>
        <AlertDialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-danger">Confirmar Exclusão de Vacinação</AlertDialogTitle>
            <AlertDialogDescription>
              Tem certeza que deseja excluir o registro de vacinação "{vaccination.vaccine}"?
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="rounded-lg bg-muted/50 p-4 space-y-2">
              <p className="text-sm font-medium">Vacinação a ser excluída:</p>
              <div className="space-y-1">
                <p className="text-sm">
                  <span className="font-semibold">Vacina:</span> {vaccination.vaccine}
                </p>
                <p className="text-sm">
                  <span className="font-semibold">Dose:</span> {vaccination.dose}
                </p>
                <p className="text-sm">
                ...
                {vaccination.batch && (
                  <p className="text-sm">
                    <span className="font-semibold">Lote:</span> {vaccination.batch}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="bg-warning/5 dark:bg-warning/20 border border-warning/30 dark:border-warning/50 rounded-lg p-4 space-y-3">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="deleteReason"
                  placeholder="Ex: Registro duplicado - Vacinação já cadastrada no sistema..."
      ...
      <ReauthenticationModal
        ...
        error={reauthError}
        actionDescription="Exclusão de registro de vacinação"
      />
    </>
  )
}

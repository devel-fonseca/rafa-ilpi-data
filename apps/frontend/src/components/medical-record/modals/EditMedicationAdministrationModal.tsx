...
export function EditMedicationAdministrationModal({
  ...
  const formId = 'edit-medication-administration-form'

  return (
    <ActionDetailsSheet
      open={open}
      onOpenChange={(nextOpen) => !nextOpen && handleClose()}
      title="Editar Administração de Medicamento"
      description="Atualize os dados e registre o motivo da edição para auditoria."
      ...
      summary={medication ? (
        <div className="bg-muted/20 p-4 rounded-lg space-y-2 border">
          ...
          {scheduledTime && (
            <div className="text-sm text-muted-foreground">
              Programado: <span className="font-medium text-foreground">{scheduledTime}</span>
            </div>
          )}
        </div>
      ) : undefined}
      ...
      footer={(
        <>
          ...
          <Button type="submit" size="sm" form={formId} disabled={isUpdating}>
            {isUpdating ? (
              <>

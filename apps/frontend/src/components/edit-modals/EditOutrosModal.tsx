...
import type { OutrosRecord } from '@/types/daily-records'
...
const editOutrosSchema = z.object({
  time: z
    .string()
...
type ComparableOutrosData = {
  descricao?: string
}

function normalizeComparableOutrosData(data: Record<string, unknown>): ComparableOutrosData {
  return {
    descricao: data.descricao ? String(data.descricao) : undefined,
  }
}

export function EditOutrosModal({
  open,
  onClose,
  onSubmit,
  record,
  isUpdating,
}: EditOutrosModalProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
    reset,
  } = useForm<EditOutrosFormData>({
    resolver: zodResolver(editOutrosSchema),
  })
  ...
  const handleFormSubmit = (data: EditOutrosFormData) => {
    ...
    const nextData: ComparableOutrosData = {
      descricao: data.descricao,
    }
    const currentData = normalizeComparableOutrosData(record.data as Record<string, unknown>)
  ...
  const formId = 'edit-outros-form'

  return (
    <ActionDetailsSheet
      open={open}
      onOpenChange={(nextOpen) => !nextOpen && onClose()}
      title="Editar Outros Registros"
